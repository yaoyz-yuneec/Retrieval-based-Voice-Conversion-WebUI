...
class AudioConfig:
    def __init__(self):
        ...
        self.samplerate = 44100  # 默认采样率，可根据模型调整
...
class PulseAudioRVC:
    ...
    async def audio_callback(self, pulse, source, sink):
        """处理音频流"""
        while self.stream_active:
            # 捕获音频
            data = await source.read_async(self.block_frame * self.audio_config.channels * 4)  # float32
            if not data:
                continue
    ...
    async def start_stream(self):
        ...
        async with PulseAsync('rvc-client') as pulse:
            # 获取默认源和接收器
            source = await pulse.source_output_new(
                source_name=None,  # 使用默认源
                stream_name='rvc-input',
            ...
            await self.audio_callback(pulse, source, sink)
...
if __name__ == "__main__":
    pulse_rvc = PulseAudioRVC()
    try:
        pulse_rvc.run()
    except KeyboardInterrupt:
        asyncio.run(pulse_rvc.stop_stream())